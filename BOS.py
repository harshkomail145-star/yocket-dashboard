        ...
        <div style="display: flex; gap: 18px; align-items: flex-start;">
            ...
            <div style="flex-shrink: 0; display: flex; align-items: center; justify-content: center; width: 42px; height: 42px; border-radius: 10px; background: #f1f5f9; border: 1px solid #e2e8f0; box-shadow: inset 0 2px 4px rgba(255,255,255,0.8); margin-top: 2px;">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="url(#ai-gradient)" width="22" height="22" style="filter: drop-shadow(0px 2px 2px rgba(0,0,0,0.1));">
                    ...
                    <path d="M19.954 8.015c-.244-.128-.535-.11-.762.046l-2.618 1.745a5.986 5.986 0 0 0-3.328-3.328L15.02 3.86a.998.998 0 0 0 .046-.762A1 1 0 0 0 14.223 2.3h-4.446a1 1 0 0 0-.843.798.998.998 0 0 0 .046.762l1.745 2.618a5.986 5.986 0 0 0-3.328 3.328L4.78 8.06a.998.998 0 0 0-.762-.046 1 1 0 0 0-.798.843v4.446a1 1 0 0 0 .798.843.998.998 0 0 0 .762-.046l2.618-1.745a5.986 5.986 0 0 0 3.328 3.328l-1.745 2.618a.998.998 0 0 0-.046.762 1 1 0 0 0 .843.798h4.446a1 1 0 0 0 .843-.798.998.998 0 0 0-.046-.762l-1.745-2.618a5.986 5.986 0 0 0 3.328-3.328l2.618 1.745c.227.156.518.174.762.046a1 1 0 0 0 .798-.843v-4.446a1 1 0 0 0-.798-.843zM12 15a3 3 0 1 1 0-6 3 3 0 0 1 0 6z"/>
                </svg>
            </div>
...
with tab_overall:
    ...
    if gemini_key and ai_master_switch:
        yoy_rubric = "In 1-2 lines, compare Fall 26 vs Fall 25 YTD volume and conversion speed. Flag if volume is up but conversion velocity is trailing last year's baseline."
        yoy_context = f"""
    ...
    if gemini_key and ai_master_switch:
        ...
        mom_rubric = "In 1-2 lines, call out MoM volume momentum across BP, Login, Sanction, and PF Paid. Highlight any stage with a differential drop vs previous months."
    ...
    # ==========================================
    if gemini_key and ai_master_switch:
        velocity_rubric = "In 1-2 lines, audit in-month processing speed vs last year. Name the exact funnel stage where pipeline friction is slowing down file movement."
    ...
    # PASTE THIS:
    render_section_header(
        "3. Fall 26 Shared Leads Cohort",
        "Shows the complete journey of all leads in the Fall 26 group. It highlights exactly how many files are currently active, successfully converted, or lost at each stage."
    )
    ...
    # ==========================================
    if gemini_key and ai_master_switch:
        cohort_rubric = "In 1-2 lines, evaluate Fall 26 conversion against baselines (BP->Log 70%, Log->San 50%, San->PF 50%). Name the stage bleeding the most leads."
        cohort_context = f"""
    ...
    # PASTE THIS:
    render_section_header(
        "4. Active Pipeline Health & Competitor Risk",
        "Evaluates the files currently being worked on by our team. It highlights how old the files are getting and flags if a competitor is moving faster than us on a shared lead."
    )
    ...
    if gemini_key and ai_master_switch:
        ...
        threat_rubric = "In 1-2 lines, audit workable files sitting >7d or untouched (zero calls). Warn where exclusive leads are slipping to competitors."
    ...
    # PASTE THIS:
    render_section_header(
        "5. Lost Leads and its Potential",
        "Analyzes the files the Lender marked as 'Lost' to see if they actually moved ahead from that stage with a competitor. This helps us understand leads with the potential that were marked as lost."
    )
    ...
    # ==========================================
    if gemini_key and ai_master_switch:
        lost_rubric = "In 1-2 lines, expose 'False Dead' files—leads tagged 'Not Interested' by RMs that went and got sanctioned/paid PF with a competitor."
   ...
   # --- SECTION 8: REGION-WISE COHORT FUNNEL (HEAT-SHADED SAAS TABLE) ---
    ...
    # PASTE THIS:
    render_section_header(
        "6. Region-Wise Cohort Funnel",
        "Breaks down our conversion success rate by specific geographic regions. This pinpoints exactly which local areas are performing well and which ones need more support."
    )
    ...
    # ==========================================
    if gemini_key and ai_master_switch:
        regional_rubric = "In 1-2 lines, compare regional hub conversion against the lender average. Name the worst-performing location and its key bottleneck stage."
