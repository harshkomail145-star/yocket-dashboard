...
@st.cache_data
def get_profile_completion_data(source, scale):
    np.random.seed(seed_map.get(source, 42))
    
    # 1. At time of Sharing (Day 0) - Usually lower because of parallel processing
    sys_avg_share_pct = np.random.uniform(45.0, 55.0)
    
    # 2. At time of Login - Should be much higher as they hunted down the docs
    sys_avg_login_pct = np.random.uniform(85.0, 96.0)
    
    return sys_avg_share_pct, sys_avg_login_pct
...
sys_avg_share_pct, sys_avg_login_pct = get_profile_completion_data(selected_source, master_scale)
...
with tab1:
    ...
    # --- UPGRADED SECTION: PROFILE COMPLETION (SHARE VS LOGIN) ---
    st.subheader("6. Profile Completion (Time of Share vs. Time of Login)")
    st.caption("Tracking how much of the student's profile is complete when we first share it with the bank, versus when it is officially logged in.")
    
    col_v1, col_v2 = st.columns(2)
    
    with col_v1:
        st.write("**Completion at Time of Sharing (Day 0)**")
        st.caption("The baseline starting point (Parallel Processing).")
        
        # The original Blue gauge
        fig_share_gauge = go.Figure(go.Indicator(
            mode = "gauge+number",
        ...
        fig_share_gauge.update_layout(template=plotly_theme, height=280, margin=dict(t=20, b=0, l=20, r=20))
        st.plotly_chart(fig_share_gauge, use_container_width=True)

    with col_v2:
        st.write("**Completion at Time of Login**")
        st.caption("Profile readiness when formally submitted for sanction.")
        
        # A Green gauge to show the successful hustle
        fig_login_gauge = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = sys_avg_login_pct,
            number = {'suffix': "%", 'font': {'color': text_color}},
            gauge = {
                'axis': {'range': [0, 100], 'tickwidth': 1, 'tickcolor': text_color},
