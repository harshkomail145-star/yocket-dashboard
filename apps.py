...
# Load and process the real data dynamically (Auto-detecting the file)
    import glob
    import os

    try:
        # Automatically finds any CSV matching your Metabase naming pattern
        csv_files = glob.glob("bank_location_wise_cohort*.csv")
        
        if not csv_files:
            st.error("⚠️ No cohort data file found! Please ensure your Metabase CSV export is placed in the app directory.")
            df_major_branches = pd.DataFrame()
        else:
            # Sorts by modification time to always pick the absolute newest download!
            latest_file = max(csv_files, key=os.path.getmtime)
            st.caption(f"📊 Feeding live data from: `{latest_file}`")
            
            df_real = pd.read_csv(latest_file)
