...
with tab_log_san:
    ...
    if not log_df.empty and 'location' in log_df.columns:
        ...
        # FIX: Replace 0 with None to prevent Plotly full-width artifacts
        u7_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(log_u7_vals, totals_health_log)]
        o7_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(log_o7_vals, totals_health_log)]
        term_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(log_term_vals, totals_health_log)]

        u7_labels = [f"{p:.0f}%" if p is not None else "" for p in u7_pct_num]
        o7_labels = [f"{p:.0f}%" if p is not None else "" for p in o7_pct_num]
        term_labels = [f"{p:.0f}%" if p is not None else "" for p in term_pct_num]
        ...
        exc_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(log_exc_vals, log_workable_totals)]
        csan_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(log_csan_vals, log_workable_totals)]

        exc_labels = [f"{p:.0f}%" if p is not None else "" for p in exc_pct_num]
        csan_labels = [f"{p:.0f}%" if p is not None else "" for p in csan_pct_num]
        ...
        res_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(res_vals, query_totals)]
        unres_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(unres_vals, query_totals)]
        res_labels = [f"{p:.0f}%" if p is not None else "" for p in res_pct_num]
        unres_labels = [f"{p:.0f}%" if p is not None else "" for p in unres_pct_num]
        ...
        td_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(true_dead_vals, lost_branch_totals)]
        cs_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(comp_san_vals, lost_branch_totals)]
        cp_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(comp_pf_vals, lost_branch_totals)]

        td_labels = [f"{p:.0f}%" if p is not None else "" for p in td_pct_num]
        cs_labels = [f"{p:.0f}%" if p is not None else "" for p in cs_pct_num]
        cp_labels = [f"{p:.0f}%" if p is not None else "" for p in cp_pct_num]
        ...
        if log_flight_risk_df.empty or 'lost_reason' not in log_flight_risk_df.columns:
            ...
            for idx, r in enumerate(top_reasons + ["Other"]):
                raw_vals = reason_data[r]
                pct_vals = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(raw_vals, branch_fr_totals)]
                labels = [f"{p:.0f}%" if p is not None else "" for p in pct_vals]
...
with tab_san_pf:
    ...
    if not san_df.empty and 'location' in san_df.columns:
        ...
        for b in san_y_branches:
            ...
            tat_days.append(round(b_df['tat_sanction_pf'].mean(), 1) if not b_df.empty and 'tat_sanction_pf' in b_df.columns and not pd.isna(b_df['tat_sanction_pf'].mean()) else 0)
        ...
        with col_c2:
            lender_avg_tat = round(san_df['tat_sanction_pf'].mean(), 1) if not san_df.empty and 'tat_sanction_pf' in san_df.columns and not pd.isna(san_df['tat_sanction_pf'].mean()) else 0
        ...
        u7_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(san_u7_vals, totals_health_san)]
        o7_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(san_o7_vals, totals_health_san)]
        term_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(san_term_vals, totals_health_san)]
        ...
        # --- ROW 3: LOSING THE ACTIVE PROSPECTS ---
        st.markdown('<div class="section-header"><h2>💸 3. Losing The Active Prospects (Branch-wise)</h2></div>', unsafe_allow_html=True)
        st.markdown("Where our **Workable** Sanction leads are sitting. *(Note: Since PF is terminal, all workable Sanction leads are 100% Exclusive)*.")

        san_exc_vals, san_workable_totals = [], []
        ...
        for b in san_y_branches:
            ...
            san_exc_vals.append(b_workable.shape[0])

        branch_loss_labels = [f"<b>{b}</b><br>{t} Workable" for b, t in zip(san_y_branches, san_workable_totals)]
        exc_pct_num = [(v/t)*100 if (t > 0 and v > 0) else None for v, t in zip(san_exc_vals, san_workable_totals)]
        exc_labels = [f"{p:.0f}%" if p is not None else "" for p in exc_pct_num]
        ...
        # --- ROW 4: QUERY RESOLUTION STATUS ---
        st.markdown('<div class="section-header"><h2>❓ 4. Query Resolution Status (Workable Sanction Leads)</h2></div>', unsafe_allow_html=True)
