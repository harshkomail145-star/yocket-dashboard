...
with tab_overall:
    ...
    # ---------------------------------------------------------
    # PART B: MONTHLY PACING CHART (JAN - AUG)
    # ---------------------------------------------------------
    st.markdown("<h4 style='color: #334155; font-size: 16px; font-weight: 700; margin-top: 10px;'>YoY Monthly Login Volume</h4>", unsafe_allow_html=True)
    ...
    fig_vol.add_trace(go.Bar(
        ...
        textfont=dict(color="#ea580c", size=12, weight="bold")
    ))

    # Fall 26: Bold Sapphire Blue
    fig_vol.add_trace(go.Bar(
    ...
    # Prevent text from getting clipped at the top of the bars
    fig_vol.update_traces(cliponaxis=False)
